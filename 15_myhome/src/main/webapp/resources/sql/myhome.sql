...
--1. 업로드 목록 (업로드 - 첨부조인, 업로드 - 사용자 조인)
--   첨부가 없는  업로드 게시글도 조회할 수 있도록 외부조인을 사용함
...
FROM (SELECT UP.UPLOAD_NO, UP.TITLE, UP.CONTENTS, UP.CREATED_AT, UP.MODIFIED_AT,
        (SELECT COUNT(*)FROM ATTACH_T ATC WHERE UP.UPLOAD_NO=ATC.UPLOAD_NO)AS ATTACH_COUNT,
        USR.USER_NO,USR.EMAIL,USR.NAME
  FROM UPLOAD_T LEFT OUTER JOIN USER_T USR
