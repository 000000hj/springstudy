...
public class MyController03 {
 ...
 // @RequestMapping("/blog/detail.do")// GET 방식의 method는 생략할 수 있다. value만 작성할 땐  value= 부분도 생략할 수 있다.
  ...
  public String blogDetail2(@RequestParam(value = "blogNo",defaultValue = "1",required = false)int blogNo,Model model) {
    model.addAttribute("blogNo",blogNo);
    return "blog/detail";
  }
   ...
   *  4) 커맨드 객체는 자동으로 Model에 저장된다. 저장될 때 객체명(dto)가 아닌 클래스명(BlogDto)으로 저장된다.(클래스명을 LowerCamelCase로 바꿔서 저장)
   */
  
  
  //@RequestMapping("/blog/detail.do")
  public String blogDetail3(BlogDto dto)// 모델에 저장된 이름은 dto가 아니라 blogDto이다.
  {
    
    return "blog/detail";
  }

//@ModelAttribute 를 이용해서 Model에 저장되는 커맨드 객체의 이름을 지정할 수 있다.
  
  @RequestMapping("/blog/detail.do")
  public String blogDetail4(@ModelAttribute("dto") BlogDto blogDto)//Model에 저장되는 이름은 dto
  {
